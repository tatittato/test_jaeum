...
# statistics crud
# "기간별" 수면 시간정보 가져오기
def get_sleep_data(start, end, nickname, db:Session, response_model: schemas.SleepInfoBase):
  # 해당하는 날짜 정보만 가져오기 (between은 filter와 함께 쓴다)
  if start == None:
    # 전체 기간동안의 정보를 가져오기
    sleep_data = db.query(model.SleepInfo).filter(model.SleepInfo.nickname == nickname).filter(model.SleepInfo.date <= end).order_by(model.SleepInfo.date).all()
  else :
    # between 기간동안의 정보를 가져오기
    sleep_data = db.query(model.SleepInfo).filter(model.SleepInfo.nickname == nickname).filter(between(model.SleepInfo.date, start, end)).order_by(model.SleepInfo.date).all()
...
# "수면 이벤트정보도 가져오기"
def get_sleep_event_data(start, end, nickname, db:Session, response_model: schemas.SleepEventBase):
  sleep_event_data = []
  if start == None:
    # 전체 기간동안의 정보를 가져오기
